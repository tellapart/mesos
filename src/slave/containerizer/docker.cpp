    ...
    pid_t pid) const
{
  ...
  if (cpuAcctStat.isError()) {
    return Error("Failed to get cpuacct.stat: " + cpuAcctStat.error());
  }
  ...
  if (flags.cgroups_enable_cfs) {
    ...
    Try<hashmap<std::string, uint64_t>> cpuStat =
      cgroups::stat(cpuHierarchy.get(), cpuCgroup.get(), "cpu.stat");
    if (cpuStat.isError()) {
      return Error("Failed to get cpu.stat: " + cpuStat.error());
    }

    Option<uint64_t> nr_periods = cpuStat.get().get("nr_periods");
    if (nr_periods.isSome()) {
      result.set_cpus_nr_periods(nr_periods.get());
    }

    Option<uint64_t> nr_throttled = cpuStat.get().get("nr_throttled");
    if (nr_throttled.isSome()) {
      result.set_cpus_nr_throttled(nr_throttled.get());
    }

    Option<uint64_t> throttled_time = cpuStat.get().get("throttled_time");
